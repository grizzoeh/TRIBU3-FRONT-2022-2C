...
export default function GannChart() {
  ...
  const handleAssigneeFilter = (e) => {
    e === "Todos"
      ? setAssignee(e)
      : setAssignee(
          assignees.find((assignee) => assignee.legajo == e).Nombre +
            " " +
            assignees.find((assignee) => assignee.legajo == e).Apellido
        );
    e === "Todos"
      ? (assigneeQuery = "")
  ...
  const handlePriorityFilter = (e) => {
    e.target.value == 0
      ? (priorityQuery = "")
      : (priorityQuery = "priority=" + e.target.value + "&");
  ...
  const getTarea = async () => {
    ...
    assigneeID === "Todos"
      ? (assigneeQuery = "")
      : (assigneeQuery = "assignee=" + assigneeID + "&");
    priority == 0
      ? (priorityQuery = "")
    ...
    axios
      ...
      .catch((err) => {
        alert(
          "Se produjo un error al consultar las tareas para el proyecto",
          err
        );
      });
    setTareas(
      tareas.filter(
        (tarea) =>
          !(
            tarea.estimated_start_date == null &&
            tarea.estimated_finalization_date != null &&
            tarea.estimated_hours_effort != null
          ) ||
          (tarea.estimated_finalization_date == null &&
            tarea.estimated_start_date != null &&
            tarea.estimated_hours_effort != null) ||
  ...
  <Button variant="primary">Volver atrás</Button>;
  // const options = {
  //     height: 600,
  //     gantt: {
    ...
    <NavbarProyectos />
      ...
      <Container className="container-title">
        ...
        <Col>
          <Link to={`/proyectos/${params.id}/ver-tareas/`}>
            <Button
              variant="primary"
              onClick={() => console.log("click diagrama gannt")}
            >
              Volver Atras
            </Button>
          </Link>
      ...
      <Container key="chart-container">
        ...
        {tareas.length > 0 ? (
          ...
          <Col class="col-xs-1" align="center">
